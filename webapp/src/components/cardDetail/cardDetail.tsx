...
const CardDetail = (props: Props): JSX.Element | null => {
    ...
    const [parent, setParent] = useState(card.parentId);
    ...
    const saveParent = useCallback(() => {
        Utils.log("Saving parent: " + parent);
        if (parent !== card.parentId) {
            mutator.changeBlockParent(props.board.id, card.id, card.parentId, parent);
        }
    ...
    useEffect(() => {
        ...
        TelemetryClient.trackEvent(TelemetryCategory, TelemetryActions.ViewCard, {
                    ...
                    <BlocksEditor
                        ...
                        onBlockCreated={async (block: any, afterBlock: any): Promise<BlockData | null> => {
                            ...
                            if (block.contentType === "checkbox") {
                                ...
                                newBlock = await addBlockNewEditor(card, intl, "", {
                                    fileId: newFileId,
                                    filename: block.value.filename
                                }, block.contentType, afterBlock?.id, dispatch);
                            } else {
                        ...
                        onBlockModified={async (block: any): Promise<BlockData<any> | null> => {
                            ...
                            mutator.updateBlock(card.boardId, newBlock, originalContentBlock, intl.formatMessage({
                                id: "ContentBlock.editCardText",
                                defaultMessage: "edit card content"
                            }));
                            return block;
                        }}
                        onBlockMoved={async (block: BlockData, beforeBlock: BlockData | null, afterBlock: BlockData | null): Promise<void> => {
                            if (block.id) {
                                ...
                                if (afterBlock && afterBlock.id) {
                                    await mutator.moveContentBlock(block.id, afterBlock.id, "after", sourceBlockId, sourceWhere, intl.formatMessage({
                                        id: "ContentBlock.moveBlock",
                                        defaultMessage: "move card content"
                                    }));
                                    return;
                                }
                                if (beforeBlock && beforeBlock.id) {
                                    await mutator.moveContentBlock(block.id, beforeBlock.id, "before", sourceBlockId, sourceWhere, intl.formatMessage({
                                        id: "ContentBlock.moveBlock",
                                        defaultMessage: "move card content"
                                    }));
                                }
