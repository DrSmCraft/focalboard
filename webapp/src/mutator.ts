...
class Mutator {
    ...
    async swapBlockParent(boardId: string, srcCard: Card, dstCard: Card, description = "swap block parent") {
        if (srcCard.parentId == dstCard.id) {
            Utils.log("Immediate parent swap!!!!!");
            // await mutator.changeBlockParent(boardId, srcCard.id, srcCard.parentId, dstCard.parentId);
            // await mutator.changeBlockParent(boardId, dstCard.id, dstCard.parentId, srcCard.id);


            await undoManager.perform(
                async () => {
                    await octoClient.patchBlock(boardId, srcCard.id, { parentId: dstCard.parentId });
                    await octoClient.patchBlock(boardId, dstCard.id, { parentId: srcCard.id });
                },
                async () => {
                    await octoClient.patchBlock(boardId, srcCard.id, { parentId: srcCard.parentId });
                    await octoClient.patchBlock(boardId, dstCard.id, { parentId: dstCard.parentId });
                },
                description,
                this.undoGroupId
            );
    ...
    async swapBlockChild(boardId: string, srcCard: Card, dstCard: Card, description = "swap block child") {
        if (srcCard.id == dstCard.parentId) {
            Utils.log("Immediate child swap!!!!!");
            // await mutator.changeBlockParent(board.id, dstCard.id, dstCard.parentId, srcCard.parentId);
            // await mutator.changeBlockParent(board.id, srcCard.id, srcCard.parentId, dstCard.id);


            await undoManager.perform(
                async () => {
                    await octoClient.patchBlock(boardId, dstCard.id, { parentId: srcCard.parentId });
                    await octoClient.patchBlock(boardId, srcCard.id, { parentId: dstCard.id });
                },
                async () => {
                    await octoClient.patchBlock(boardId, dstCard.id, { parentId: dstCard.parentId });
                    await octoClient.patchBlock(boardId, srcCard.id, { parentId: srcCard.parentId });
    ...
    async changePropertyValue(boardId: string, card: Card, propertyId: string, value?: string | string[], description = "change property") {
        ...
        TelemetryClient.trackEvent(TelemetryCategory, TelemetryActions.EditCardProperty, {
            board: card.boardId,
            card: card.id
        });
    }
    ...
    async addBoardFromTemplate(
        ...
        const actionDescription = intl.formatMessage({
            id: "Mutator.new-board-from-template",
            defaultMessage: "new board from template"
        });
    ...
    async addEmptyBoardTemplate(
        ...
        boardTemplate.title = intl.formatMessage({
